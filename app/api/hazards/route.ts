...
import type { CreateHazardInput, HazardSeverity } from "@/lib/types/hazard"
import { randomUUID } from "crypto"

export const dynamic = "force-dynamic"

function isSeverity(value: unknown): value is HazardSeverity {
  return value === "low" || value === "medium" || value === "high"
}

function parseHazardFields(formData: FormData): Partial<CreateHazardInput> & { lat: number; lng: number } | null {
  ...
  return {
    ...
    locationDescription:
      typeof locationDescription === "string" && locationDescription ? locationDescription : undefined,
    emergency: emergency === "true" || emergency === "1",
  }
}

function validateJsonBody(body: Partial<CreateHazardInput>): CreateHazardInput | null {
  if (!body.type || typeof body.type !== "string") return null
  if (!isSeverity(body.severity)) return null
...
export async function GET() {
  return NextResponse.json({ hazards: listHazards() })
}
...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as Partial<CreateHazardInput>
    const input = validateJsonBody(body)
    if (!input) {
      return NextResponse.json({ error: "Invalid request body" }, { status: 400 })
    }

    const hazard = createHazard(input)
    return NextResponse.json({ hazard }, { status: 201 })
  } catch (e) {
