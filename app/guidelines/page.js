        ...
        <main className="min-h-screen bg-white text-slate-900">
            <div className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
                <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-sm text-slate-500">
                    <Link href="/" className="inline-flex items-center gap-1 font-medium text-blue-700 hover:underline"><Home className="h-4 w-4" /> Home</Link>
                ...
                <header className="mt-8 border-b border-slate-200 pb-8">
                    <p className="text-sm font-bold uppercase tracking-[0.16em] text-blue-700">Guidelines</p>
                    <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Hazard safety guidelines</h1>
                    <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">Practical, public-facing advice for safely identifying, reporting, and responding to hazards. Review official local instructions during an active emergency.</p>
                </header>

                <div className="mt-8 grid gap-10 lg:grid-cols-[13rem_minmax(0,1fr)]">
                    <aside className="h-fit border-l-2 border-blue-100 pl-4 text-sm lg:sticky lg:top-6">
                        <p className="font-bold text-slate-900">On this page</p>
                        <ol className="mt-3 space-y-2 text-slate-600">
                            {sections.map((section) => <li key={section.number}><a className="hover:text-blue-700 hover:underline" href={`#section-${section.number}`}>{section.number}. {section.title}</a></li>)}
                        </ol>
                        <Link href="/guidelines/workflow" className="mt-6 block font-semibold text-blue-700 hover:underline">View application walkthrough →</Link>
                    </aside>

                    <article>
                        {sections.map((section) => (
                            <section key={section.number} id={`section-${section.number}`} className="scroll-mt-6 border-b border-slate-200 py-8 first:pt-0 last:border-b-0">
                                <h2 className="flex items-center gap-3 text-xl font-bold text-slate-950"><span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-blue-950 px-2 text-sm text-white">{section.number}</span>{section.title}</h2>
                                <div className="mt-5">
                                    {section.items.map(([heading, text]) => <div key={heading} className="border-b border-slate-200 py-5 first:pt-0 last:border-b-0 last:pb-0"><h3 className="font-semibold text-slate-900">{heading}</h3><p className="mt-2 max-w-3xl leading-7 text-slate-600">{text}</p></div>)}
                                </div>
                            </section>
                        ))}
                    </article>
