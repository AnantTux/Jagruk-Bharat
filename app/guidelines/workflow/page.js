        ...
        <main className="min-h-screen bg-white text-slate-900">
            <div className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
                <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-sm text-slate-500">
                    <Link href="/" className="inline-flex items-center gap-1 font-medium text-blue-700 hover:underline"><Home className="h-4 w-4" /> Home</Link><ChevronRight className="h-4 w-4" /><Link href="/guidelines" className="font-medium text-blue-700 hover:underline">Guidelines</Link><ChevronRight className="h-4 w-4" /><span className="text-slate-800">Application walkthrough &amp; workflow</span>
                </nav>
                <header className="mt-8 border-b border-slate-200 pb-8"><p className="text-sm font-bold uppercase tracking-[0.16em] text-blue-700">Guidelines</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Application walkthrough &amp; workflow</h1><p className="mt-3 max-w-3xl leading-7 text-slate-600">A concise guide to using Jagruk Bharat, from viewing the map to responsible moderation.</p></header>
                <article className="mt-8 max-w-4xl">
                    {sections.map(([number, title, items]) => <section key={number} className="border-b border-slate-200 py-8 first:pt-0 last:border-b-0"><h2 className="flex items-center gap-3 text-xl font-bold text-slate-950"><span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-blue-950 px-2 text-sm text-white">{number}</span>{title}</h2><div className="mt-5">{items.map(([heading, text]) => <div key={heading} className="border-b border-slate-200 py-5 first:pt-0 last:border-b-0 last:pb-0"><h3 className="font-semibold text-slate-900">{heading}</h3><p className="mt-2 max-w-3xl leading-7 text-slate-600">{text}</p></div>)}</div></section>)}
                </article>
            </div>
        </main>
    );
