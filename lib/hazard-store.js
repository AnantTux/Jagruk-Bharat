...
export async function deleteHazardForRateLimit(id, userId) {
    await connectToDatabase();
    const result = await Hazard.deleteOne({ id, reportedByUserId: userId });
    if (result.deletedCount > 0) {
        await deleteCached(HAZARD_LIST_CACHE_KEY);
        publishHazardUpdate({ action: "deleted", hazardId: id });
    }
}
