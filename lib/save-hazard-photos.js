...
import { uploadHazardPhoto } from "@/lib/cloudinary";
const MAX_PHOTOS = 5;
...
export async function saveHazardPhotos(files, hazardId) {
    ...
    const useLocalStorage = !useBlobStorage &&
        (process.env.NODE_ENV !== "production" || process.env.LOCAL_FILE_UPLOADS === "true");
    if (!useBlobStorage && !useCloudinary && !useLocalStorage) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (useCloudinary) {
            urls.push(await uploadHazardPhoto(sanitizedBuffer, hazardId, filename));
        }
